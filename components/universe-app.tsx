...
function Home({ profile, boards, memories, notes, onBoard, onMemory, onCreate }: any) { return <section className="screen"><Header title={`Good evening, @${profile.username}`} action={<button className="icon-button"><Bell size={18} /></button>} /><p className="lede">Your little universe, in pieces.</p><section className="hero-note"><div><span className="eyebrow">A SMALL REMINDER</span><p>Collecting is a way of paying attention.</p></div><Sparkles size={22} /></section><SectionTitle title="Recent memories" action="View archive" /><div className="memory-row">{memories.slice(0, 3).map((m: Memory) => <MemoryCard key={m.id} memory={m} onClick={() => onMemory(m)} />)}</div><SectionTitle title="Your collections" action="See all" /><div className="board-grid">{boards.slice(0, 4).map((b: Board) => <BoardCard key={b.id} board={b} onClick={() => onBoard(b)} />)}</div><SectionTitle title="A thought to keep" /><div className="note-card"><FileText size={17} /><p>{notes[0]?.text || 'Your universe is still empty.'}</p><span>{notes[0]?.date}</span></div><button className="quick-add" onClick={onCreate}><Plus size={18} /> Add something to remember</button></section> }
